...
function findElementGreaterThan(arr, num) {
  if (Array.isArray(arr) === true) {
    const elem = arr.find((number) => number > num);
    if (elem !== undefined) {
      return elem;
    } else {
...
function findIndexByProperty(arr, name) {
  if (Array.isArray(arr) === true) {
    const index = ({ name: newname }) => name === newname;
...
function findLastEvenNumber(arr) {
  if (Array.isArray(arr) === true) {
    const lastEvenNumber = arr.findLast((arr) => arr % 2 === 0);

    // if (arr.findLast(lastEvenNumber)) {
    //   return arr.findLast(lastEvenNumber);
    // } else "Вхідний параметр має бути масивом";
    // return arr.findLast(lastEvenNumber);
...
function getSubArrayAndConvertToString(arr, startIdx) {
  if (Array.isArray(arr) === true) {
    const index = arr.slice(startIdx);
    return index.join(" ");
  } else {
...
function getArrayKeys(arr) {
  if (Array.isArray(arr) === true) {
    const arrNew = Array.from(arr.keys());
    return arrNew;
  } else {
    return "Вхідний параметр має бути масивом";
  }
...
function getPositiveArrayValues(arr) {
  if (Array.isArray(arr) === true) {
    const arrNew = arr.values();
    const arrNewIt = Array.from(arrNew);
    const result = arrNewIt.filter((num) => num > 0);
    return result;
  } else {
    return "Вхідний параметр має бути масивом";
  }
...
function removeAndDouble(arr, startIdx) {
  if (Array.isArray(arr) === true) {
    arr.splice(startIdx);
    const result = arr.map((rem) => rem * 2);
    return result;
  } else {
    return "Вхідний параметр має бути масивом";
  }
...
function sumAndPrint(arr) {
  if (Array.isArray(arr) === true) {
    const sum = arr.reduce((sum, print) => sum + print);
...
function getLastElementsSumAndPrint(arr, n) {
  if (Array.isArray(arr) === true) {
    const last = arr.slice(n * -1);
    const sum = last.reduceRight((l1, l2) => l1 + l2);
    last.forEach((n) => console.log(n));
    return sum;
  } else {
    return "Вхідний параметр має бути масивом";
  }
...
function wordLengths(arr) {
  if (Array.isArray(arr) === true) {
    const elem = arr.map((word) => word.length);
    return elem;
  } else {
    return "Вхідний параметр має бути масивом";
  }
...
function findLongestWord(arr) {
  if (Array.isArray(arr) === true) {
    const long = arr.reduce((word, wordLonges) =>
      word.length < wordLonges.length ? (wordLonges = wordLonges) : wordLonges
    );
    return long;
  } else {
    return;
...
function findDuplicateElements(arr) {
  if (Array.isArray(arr) === true) {
    const dup = arr.filter((elem, index) => arr.indexOf(elem) - index);
    // console.log(dup.indexOf(elem));
    return dup;
  } else {
    return;
  }
...
function capitalizeWords(arr) {
  if (Array.isArray(arr) === true) {
    const arrNew = arr.map((word) => word[0].toUpperCase() + word.slice(1));
...
function calculateTotalPrice(arr) {
  if (Array.isArray(arr) === true) {
    const sum = arr.reduce((num, num2) => num + num2.price, 0);
...
function findLastNegativeNumberIndex(arr) {
  if (Array.isArray(arr) === true) {
    const last = arr.findLastIndex((num, num2) => num + num2);
    return last;
  } else {
    return -1;
  }
